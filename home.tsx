...
export default function Home() {
    ...
    return (
        <main className="bg-gray-900 text-white font-sans overflow-x-hidden">
            {/* ---------- Hero 封面 ---------- */}
            <section
                className="h-[90vh] flex items-center justify-center bg-cover bg-center text-center"
                ...
                <div className="bg-black/40 p-10 rounded-xl">
                    ...
                    <p className="text-gray-300 text-lg max-w-2xl mx-auto">
                        {isChinese
                            ...
                            : "In the ancient town of Lili, a Ming-era residence glows softly like pewter, tranquil and timeless."}
                    </p>
                </div>
            </section>
            ...
            <SectionScroll
                id="exhibitions"
                title={isChinese ? "展览与收藏" : "Exhibitions & Collections"}
                desc={
                    isChinese
                        ? "馆藏横跨明清至今，涵盖生活器具、宗教用器与当代艺术作品。"
                        : "The museum’s collection spans from the Ming and Qing dynasties to today."
                }
            ...
            <SectionScroll
                id="events"
                title={isChinese ? "活动与交流" : "Events & Partnerships"}
                desc={
                    isChinese
                        ? "从区域峰会到国际展会，博物馆积极参与文化交流与国际展览，让锡的温度跨越语言与国界。"
                        : "From regional summits to international fairs, the museum brings Chinese pewter to the world."
                }
                items={[
                    {
                        id: 1,
            ...
            <SectionScroll
                id="products"
                title={isChinese ? "文创用品" : "Cultural & Creative Products"}
                desc={
                    isChinese
                        ? "以锡为媒介，延伸生活之美与审美日常。"
                        : "Pewter-inspired creations that bring art into everyday life."
                }
                items={[
                    {
                        id: 1,
            ...
            <SectionScroll
                id="learning"
                title={isChinese ? "研学与教育" : "Learning & Discovery"}
                desc={
                    isChinese
                        ? "如果说展厅是时间的剧场，那么研学，就是让观众成为演员的舞台。"
                        : "If the exhibition hall is a theatre of time, Learning is where visitors become performers."
                }
                items={[
                    {
                        id: 1,
...
function SectionScroll({
    ...
    return (
        <div id={id} className="relative z-10 bg-[#0f172a] py-24 text-center fade">
            <h2 className="text-3xl md:text-5xl font-bold mb-8 leading-snug">{title}</h2>
            {desc && (
            ...
            <div className="relative">
                ...
                <button
                    id={`scrollLeft-${id}`}
                    className="absolute left-0 top-1/2 -translate-y-1/2 bg-red-700/50 text-white p-3 rounded-full z-20 hover:bg-red-600/70 transition"
                >
                    ‹
                </button>
                <button
                    id={`scrollRight-${id}`}
                    className="absolute right-0 top-1/2 -translate-y-1/2 bg-red-700/50 text-white p-3 rounded-full z-20 hover:bg-red-600/70 transition"
                >
                    ›
                </button>
                ...
                <div
                    ...
                    className="flex overflow-x-scroll scroll-smooth space-x-6 px-10 pb-4 no-scrollbar"
                >
                    {items.map((item) =>
                        hasLink ? (
                            <a
                                key={item.id}
                                href={item.link}
                                className="flex-shrink-0 w-[360px] bg-[#13203b] hover:bg-[#182a4d] transition-all duration-300 shadow-lg"
                            >
